...
class CircleApiIntegrationTests {
    ...
    private static final long CIRCLE_ID = 50L;
    ...
    void postsForbiddenBeforeJoin() throws Exception {
        mockMvc.perform(get("/api/v1/circles/" + CIRCLE_ID + "/posts")
    ...
    void joinThenReadAndCreatePost() throws Exception {
        mockMvc.perform(post("/api/v1/circles/" + CIRCLE_ID + "/join")
                        .header("authorization", memberToken))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(post("/api/v1/circles/" + CIRCLE_ID + "/posts")
                        .header("authorization", memberToken)
